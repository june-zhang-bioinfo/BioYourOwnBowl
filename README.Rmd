...
**BioYourOwnBowl** is an R package for single-cell RNA-seq analysis based on Seurat.  
It provides automated workflow to generate nuanced clustering and comprehensive visualizations to assist with evaluation. This pipeline is detachable, each function can work individually, allowing users to assemble their own pipeline.
...
- **Clustering optimization**: parameter sweeps and save desired grabularity only.
- **Advanced visualizations**: cell-level, pseudobulk-level and meta-level heatmaps, double volcano plots, density plot with scanpy, violin plots with pronounced non-zero distribution, area proportional venn plots and treemaps.
...
- `prepare_h5ad()` - Export to AnnData format, used for Python-based functions
- `density_plot()` - Generate Scanpy density plot in your RStudio

## Full Workflow

For a complete example workflow with detailed explanations, see the [package vignette](https://june-zhang-bioinfo.github.io/BioYourOwnBowl/).
