...
features_plots <- function(object,
  ...
  if (!is.null(file_name)) {
    png(paste0(file_name, "_original.png"), width = width_scaled, height = height_scaled, res = 200)
    print(p_original)
    dev.off()
  }
  ...
  # Save scaled plots
  if (!is.null(file_name)) {
    png(paste0(file_name, "_scaled.png"), width = width_scaled, height = height_scaled, res = 200)
...
#' across different cell identities in a Seurat object.
#'
...
#' @param height Numeric value for plot height in inches. Default is 15.
...
#' )
#'
#' # With specific groups and colors
#' violin_plots(
#'   object = object,
#'   features = gene_list,
#'   idents = "cohort_tp",
...
violin_plots <- function(object,
  ...
  # Set active identity
  levels <- object[[idents]]%>% unlist() %>% as.character() %>% str_sort() %>% unique()
...
#'   "wilcox" (Wilcoxon rank-sum test), "t.test" (t-test), or "kruskal" (Kruskal-Wallis test
#'   for more than 2 groups). Default is "wilcox".
